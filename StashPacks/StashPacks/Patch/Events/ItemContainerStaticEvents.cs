...
namespace ModifAmorphic.Outward.StashPacks.Patch.Events
{
    internal static class ItemContainerStaticEvents
    {
        ...
        public static void RaiseBagContentsChangedAfter(Bag bag, ContentChanges contentChange)
        {
            ...
            {
                Logger?.LogException($"Exception in {nameof(SaveInstanceEvents)}::{nameof(RaiseBagContentsChangedAfter)}.", ex);
                if (Logger == null)
                    UnityEngine.Debug.LogError($"Exception in {nameof(SaveInstanceEvents)}::{nameof(RaiseBagContentsChangedAfter)}:\n{ex}");
            }
        }
    }
}
