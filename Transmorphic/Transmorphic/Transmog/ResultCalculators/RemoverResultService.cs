...
namespace ModifAmorphic.Outward.Transmorphic.Transmog
{
    internal class RemoverResultService : IDynamicResultService
    {
        ...
        public void CalculateResult(DynamicCraftingResult craftingResult, Recipe recipe, IEnumerable<CompatibleIngredient> ingredients)
        {
            ...
            if (removalTarget == null || !removalTarget.IsTransmogIngredient())
            {
                Logger.LogDebug($"{nameof(RemoverResultService)}::{nameof(CalculateResult)}: Failed to Calculate result for " +
                    $"Dynamic Result ItemID {craftingResult.ItemID} and recipe {recipe.Name}. No Armor, Weapons, Bag or Lexicon found in {ingredients.Count()} ingredients.");
